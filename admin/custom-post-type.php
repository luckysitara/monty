...
add_action('save_post', 'venom_save_profile_image_data'); // Added action for saving profile picture
...
function venom_set_columns_name($columns)
{
    $clientColumns = array(
        ...
        'profile_image' => 'Profile Picture', // Added column for profile picture
...
function venom_custom_columns($columns, $post_id)
{
    switch ($columns) {
        ...
        case 'profile_image':
            $value = get_post_meta($post_id, '_venom_profile_image_value_key', true);
            echo '<img src="' . esc_attr($value) . '" style="max-width: 100px; height: auto;" />';
            break;
...
function venom_add_meta_box()
{
    ...
    add_meta_box('venom_profile_image', 'Profile Picture', 'venom_profile_image_callback', 'venom', 'normal'); // New meta box for profile picture
}
...
function venom_profile_image_callback($post)
{
    wp_nonce_field('venom_save_profile_image_data', 'venom_profile_image_meta_box_nonce');
    $value = get_post_meta($post->ID, '_venom_profile_image_value_key', true);

    echo '<label for="venom_profile_image_field"> Profile Picture </label><br><br> ';
    echo '<input type="text" name="venom_profile_image_field" id="venom_profile_image_field" value="' . esc_attr($value) . '" size="25"/>';
    echo '<input type="button" id="venom_upload_image_button" class="button" value="Upload Image" />';
    echo '<script>
        jQuery(document).ready(function($){
            $("#venom_upload_image_button").click(function(e) {
                e.preventDefault();
                ...
                custom_uploader.on("select", function() {
                    ...
                    $("#venom_profile_image_field").val(attachment.url);
                });
...
function venom_save_profile_image_data($post_id)
{
    if (!isset($_POST['venom_profile_image_meta_box_nonce'])) {
        return;
    }
    if (!wp_verify_nonce($_POST['venom_profile_image_meta_box_nonce'], 'venom_save_profile_image_data')) {
        return;
    }
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
        return;
    ...
    if (!isset($_POST['venom_profile_image_field'])) {
        return;
    }

    $my_data = sanitize_text_field($_POST['venom_profile_image_field']);

    update_post_meta($post_id, '_venom_profile_image_value_key', $my_data);
}
